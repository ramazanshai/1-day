...
function show_board() {
    ...
    html += "<td class='kazan' rowspan='4'>" + createBalls(toguzFields[19]) + "</td>";
    for (var i = 9; i > 0; i--)
        html += "<td>" + i + "</td>";
    html += "<td class='kazan' rowspan='4'>" + createBalls(toguzFields[18]) + "</td>";
    html += "</tr>";
    ...
    for (var i = 17; i > 8; i--) {
        if (toguzFields[i] == 255) {
            ...
            html += "<td onclick='click_otau(" + i + "); '>" + createBalls(toguzFields[i]) + "</td>";
        }
    }
...
function show_moves() {
    ...
    for (var i = 0; i < gameMoves.length; i++) {
        if (i % 20 === 0) {
            if (i !== 0) {
                html += '</div>'; // Закрыть предыдущий контейнер
            }
            html += '<div class="move-block">'; // Открыть новый контейнер
        }
    ...
    html += '</div>'; // Закрыть последний контейнер
